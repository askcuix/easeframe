...
/**
 * 模拟TestNG Groups分组执行用例功能的TestRunner.
 * 
 * 比如可以在白天持续执行速度较快的测试方法, 到了晚上再执行较慢的测试方法.
 * 
 * Runner会只执行测试类的@Groups定义, 在JVM运行参数-Dtest.groups=xxx,yyy中的测试类及测试方法.
 * 如无-Dtest.groups定义则执行全部测试用例.
...
public class GroupsTestRunner extends BlockJUnit4ClassRunner {
	 ...
	 * eg. java -Dtest.groups=Daily,Nightly
	 * 如果无定义则返回Groups.ALL.
	 */
